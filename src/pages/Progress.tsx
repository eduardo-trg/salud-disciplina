...
export default function Progress() {
  ...
  const handleExport = async () => {
    const reportText = generateReport(logs, 'Usuario Registrado', reportRange);
    try {
  ...
  if (loading) return <div className="min-h-screen bg-gray-50 flex items-center justify-center"><p className="text-gray-600">Cargando tu progreso...</p></div>;
  ...
  const totalMinutes = logs.reduce((sum, log) => sum + (log.activities || []).reduce((a, act) => a + (act.minutes || 0), 0), 0);
  const avgEnergy = logs.length > 0 ? (logs.reduce((sum, log) => sum + (log.wellness?.energy || 0), 0) / logs.length).toFixed(1) : '0.0';
      ...
      <main className="max-w-md mx-auto px-4 pt-6">
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-xl p-3 mb-4 transition-all duration-200">
          ...
          <div className="flex gap-2">
            <button
              ...
              className={`flex-1 py-2 rounded-lg text-sm font-medium transition-all duration-150 active:scale-[0.98] ${reportRange === 7 ? 'bg-blue-600 text-white' : 'bg-white text-blue-700 border border-blue-200 hover:bg-blue-50'}`}
            >
              Últimos 7 días
            </button>
            <button
              onClick={() => setReportRange(30)}
              className={`flex-1 py-2 rounded-lg text-sm font-medium transition-all duration-150 active:scale-[0.98] ${reportRange === 30 ? 'bg-blue-600 text-white' : 'bg-white text-blue-700 border border-blue-200 hover:bg-blue-50'}`}
            >
              Últimos 30 días
            </button>
          </div>
        ...
        <div className="space-y-4 mb-6">
          {logs.slice(0, 7).map((log) => {
            const dayMinutes = (log.activities || []).reduce((sum, a) => sum + (a.minutes || 0), 0);
            ...
            return (
              <div key={log.id} className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 transition-all duration-200 hover:shadow-md hover:-translate-y-0.5">
        ...
        <button
          ...
          className="w-full bg-gray-900 hover:bg-gray-800 active:bg-gray-700 disabled:bg-gray-300 disabled:cursor-not-allowed text-white font-semibold py-3 px-4 rounded-xl shadow-md transition-all duration-150 active:scale-[0.98] focus:outline-none focus:ring-4 focus:ring-gray-400"
        >
        ...
        <div className="mt-6 bg-green-50 border border-green-200 rounded-xl p-4 transition-all duration-200 hover:bg-green-100">
          <p className="text-sm text-green-800">
            💡 <strong>Recuerda:</strong> La constancia supera a la perfección. Un día "regular" sigue sumando a tu tendencia general.
          </p>
        </div>
      </main>
      <BottomNav />
